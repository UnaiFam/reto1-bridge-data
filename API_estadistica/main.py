...
# Carga con filtros (Mongo) por colección

def _build_filter_fechas_cabecera(start_date: str|None, end_date: str|None):
    """Para colecciones Combustible / Electricos (fechaEmision en cabecera)."""
    f = {}
...
def _build_filter_fechas_toll(start_date: str|None, end_date: str|None):
    """Para Peaje (fechaHora ya combinada)."""
    f = {}
    if start_date or end_date:
        rango = {}
        if start_date:
...
def load_df_mongo(collection, filtros: dict):
...
def add_time_cols_fuel_ev(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "horaEmision" in df.columns:
        df["dt"] = pd.to_datetime(df["fechaEmision"].astype(str) + " " + df["horaEmision"].fillna("00:00:00"), errors="coerce")
    else:
        df["dt"] = df["fechaEmision"]
...
# Aplanado de líneas y KPI builders
...
# ---------- KPIs Combustible (usuario/empresa) ----------
def kpis_usuario_fuel(df_tickets: pd.DataFrame, df_lines: pd.DataFrame):
    ...
    dias_mediana = df_tickets.groupby("idUsuario")["dt"].apply(_days_diff_mediana).reset_index(name="dias_mediana")
    anomalias_usuario = (
...
def kpis_usuario_toll(df: pd.DataFrame):
    ...
    def _days_diff_mediana(series_dt):
        ...
        d = np.diff(s).astype("timedelta64[D]").astype(float)
        return float(np.median(d))
    dias_mediana = dfu.groupby("idUsuario")["fechaHora"].apply(_days_diff_mediana).reset_index(name="dias_mediana")
...
def kpis_empresa_toll(df: pd.DataFrame):
    dfe = df.copy()
    if "empresaNombre" in dfe.columns:
        dfe["empresa"] = dfe["empresaNombre"]
    else:
        dfe["empresa"] = "EMPRESA_UNICA"
...
# Helpers para serializar dataframes
...
# Endpoints con filtros (fecha, empresa, idUsuario)
# Formato fechas: YYYY-MM-DD

@app.get("/kpis/combustible")
def kpis_combustible(
...
@app.get("/")
def root():
    return {"message": "API KPIs lista. Endpoints: /kpis/combustible, /kpis/ev, /kpis/peaje"}
